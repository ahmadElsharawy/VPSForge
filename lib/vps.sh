...
export_vps_backup() {
  ...
  sync_vps_metadata "$name"
  mkdir -p "$BACKUP_DIR"
  default_name="${name}-backup-$(date +%Y%m%d-%H%M%S).tar.gz"
  read -r -p "Enter backup filename [default: $default_name]: " custom_name </dev/tty
  if [ -n "$custom_name" ]; then
    [[ "$custom_name" == *.tar.gz ]] || custom_name="${custom_name}.tar.gz"
...
rename_backup_file() {
  if select_backup_file "Select backup file to RENAME: "; then
    ...
    read -r -p "Enter new backup filename [e.g. custom-backup.tar.gz]: " new_name </dev/tty
    [ -n "$new_name" ] || { echo "Rename cancelled."; return 0; }

    [[ "$new_name" == *.tar.gz ]] || new_name="${new_name}.tar.gz"
